        ...
        const std::vector<std::array<double,2>>& baseAirfoil)
{
    ...
    for(int i=0; i<NR; ++i)
    {
        ...
        // angle around blade (fanwise)
        // each airfoil section spans the same θ positions
        for(int j=0; j<NP; ++j)
        {
            ...
            // -----------------------------------
            double theta = 0.0;  // <-- single blade, no azimuth rotation
            ...
            Vec3 e_r = { cr, sr, 0.0 };
            Vec3 e_t = { -sr, cr, 0.0 };
            Vec3 e_z = { 0.0, 0.0, 1.0 };

            // map into global XYZ
            Vec3 P;
            P[0] = r[i] * e_r[0] + xr * e_t[0] + yr * e_z[0];
            P[1] = r[i] * e_r[1] + xr * e_t[1] + yr * e_z[1];
            P[2] = r[i] * e_r[2] + xr * e_t[2] + yr * e_z[2];
        ...
        const std::vector<std::vector<Vec3>>& bladeRings)
{
    ...
    // --------------------------------------------------
    // 1) Surface: connect all rings, including wrap-around
    // --------------------------------------------------
    for(int i=0; i<NR-1; ++i){
        for(int j=0; j<NP; ++j){
            int jn = (j+1) % NP;  // wrap around

            Vec3 p1 = bladeRings[i][j];
            Vec3 p2 = bladeRings[i+1][j];
            Vec3 p3 = bladeRings[i+1][jn];
            Vec3 p4 = bladeRings[i][jn];
    ...
    // 2) Root cap (i = 0)
    // --------------------------------------------------
    // Choose center = average of ring 0
    Vec3 rootCenter = {0,0,0};
    for(int j=0;j<NP;++j){
        rootCenter[0] += bladeRings[0][j][0];
        rootCenter[1] += bladeRings[0][j][1];
        rootCenter[2] += bladeRings[0][j][2];
    }
    rootCenter[0] /= NP;
    rootCenter[1] /= NP;
    rootCenter[2] /= NP;
    ...
    for(int j=0; j<NP; ++j){
        int jn = (j+1) % NP;
        // orientation chosen to point outward (check and flip if needed)
        writeTri(rootCenter, bladeRings[0][jn], bladeRings[0][j]);
    }

    // --------------------------------------------------
    // 3) Tip cap (i = NR-1)
    // --------------------------------------------------
    Vec3 tipCenter = {0,0,0};
    for(int j=0;j<NP;++j){
        tipCenter[0] += bladeRings[NR-1][j][0];
        tipCenter[1] += bladeRings[NR-1][j][1];
        tipCenter[2] += bladeRings[NR-1][j][2];
    }
    tipCenter[0] /= NP;
    tipCenter[1] /= NP;
    tipCenter[2] /= NP;
    ...
    for(int j=0; j<NP; ++j){
        int jn = (j+1) % NP;
        // orientation chosen opposite of root cap
        writeTri(tipCenter, bladeRings[NR-1][j], bladeRings[NR-1][jn]);
    }
...
void computeChordForConstantGamma(
        ...
        double Lprime_root, double Lprime_tip, // linear lift endpoints
        ...
        double rpm, std::vector<double> px, std::vector<double> py)
{
    ...
    for (int m = 0; m < N; ++m)
    {
        ...
        // ---- LINEAR SECTIONAL LIFT TARGET ----
        // L'(r) interpolated linearly between endpoints
        double Lprime_r = Lprime_root + (Lprime_tip - Lprime_root) * frac;
        ...
        // clamp for manufacturability near root if desired
        //if (c_local < c_min) c_local = c_min;
        //if (c_local > c_max) c_local = c_max;
...
int main()
{
    try {
        ...
        int resolution = 100;      // number of sample points around circle -> approximate panels
        double disX = 0.06;        // x-offset of circle center (negative/positive shapes camber)
        double disY = 0.20;        // y-offset of circle center
        double backFat = 1.08;     // 'backFat' scaling parameter used in generation
        double AoA_deg = 0.0;    // angle of attack in degrees (used as alpha root default)

        // blade radial / lifting-line parameters
        double r0 = 0.04;          // hub radius (m)
        double r1 = 0.14;           // tip radius (m)
        int nRadial = 100;           // number of radial collocation stations for lifting-line
