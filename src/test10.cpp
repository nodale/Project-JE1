        ...
        double rpm)
{
    ...
    // -------------------------
    // Chord calculation with linear lift
    // -------------------------
    chord_out.resize(N);
    ...
    for (int m = 0; m < N; ++m) {
        ...
        double Gamma_r = Lprime_r / Vloc; // circulation
        double c_local = (8.0 * M_PI * r * Gamma_r) / (Vloc * denom);
        //c_local = std::max(c_min, std::min(c_max, c_local));

        chord_out[m] = c_local;
        Vlocal[m] = Vloc;

        std::cout << "r=" << r << " Vlocal=" << Vloc << " chord=" << c_local << " Lprime=" << Lprime_r << std::endl;
    }
...
int main()
{
    try {
        ...
        double alpha_root_deg = 25.0;
        double alpha_tip_deg = 20.0; 
