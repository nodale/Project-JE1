...
void computeChordForConstantGamma(double r0, double r1, int Ncoll,
    ...
    ```
    double s = r1 - r0;    // blade semi-span (hub → tip)
    double b = 2.0 * s;    // full span equivalent
    ...
    // Linear geometric twist distribution
    alpha_out.assign(N, 0.0);
    for (int m = 0; m < N; ++m) {
        double frac = (r_out[m] - r0) / (r1 - r0);
    ...
    // Peak value such that average lift = Lprime_const_Nperm
    // ∫ Γ(r) dr = (π/4)*s*Γ_peak
    double Gamma_peak = (Lprime_const_Nperm / (rho * Vinf)) * (4.0 / (PI * s));

    for (int m = 0; m < N; ++m) {
        double frac = (r_out[m] - r0) / s;  // 0..1
        Gamma[m] = Gamma_peak * std::sqrt(1.0 - frac * frac);
    }
    ...
    for (int k = 0; k < N; ++k) {
        ...
        // discrete sine projection
        for (int m = 0; m < N; ++m) {
            sum += Gamma[m] * std::sin(n * theta[m]);
        }

        // normalization for lifting-line
        A[k] = (2.0 / (N + 1)) * sum / (b * Vinf);
    }

    // -------------------------
    //     Solve chord = μ b
    // -------------------------
    const double sin_eps = 1e-8, denom_eps = 1e-9;
    ...
    for (int m = 0; m < N; ++m) {
        ...
        for (int k = 0; k < N; ++k) {
            int n = nlist[k];
            double sn = std::sin(n * th);
            S1 += A[k] * sn;
            S2 += A[k] * sn * (double(n) / sinth_safe);
        }

        double alpha_rad = alpha_out[m] * DegreeToRad;
        double alpha_L0 = 0.0;   // zero-lift angle (assume 0 unless you change this)

        double denom = 2.0 * (alpha_rad - alpha_L0) + S2;
        if (std::abs(denom) < denom_eps)
            denom = (denom >= 0.0) ? denom_eps : -denom_eps;

        double mu = (4.0 * S1) / denom;
        double c_local = b * mu;

        if (std::isnan(c_local) || !std::isfinite(c_local))
            c_local = c_min;
...
int main()
{
    try {
        ...
        double AoA_deg = -25.0;    // angle of attack in degrees (used as alpha root default)

        // blade radial / lifting-line parameters
        double r0 = 0.04;          // hub radius (m)
        double r1 = 0.12;           // tip radius (m)
        int nRadial = 400;           // number of radial collocation stations for lifting-line
        ...
        computeChordForConstantGamma(r0, r1, nRadial, Vinf, rho,
