        ...
        std::vector<double>& alpha_out)
{
    ...
    //double Gamma_hub = (Lprime_const_Nperm / (rho * Vinf)) * (2.0 / s); // approximate total lift scaling
    double Gamma_hub = Lprime_const_Nperm / (rho * Vinf);
    double Gamma_tip_frac = 0.1; // fraction of hub value at tip

    for (int m = 0; m < N; ++m) {
        double frac = (r_out[m] - r0) / s;   // 0 at hub, 1 at tip
        Gamma[m] = Gamma_hub * (Gamma_tip_frac + (1.0 - Gamma_tip_frac) * pow(1.0 - frac, 2));
    ...
    std::cout << "Wrote constant_gamma_chord.dat (" << N
        << " stations) using linear Gamma(r) distribution\n";
...
int main()
{
    try {
        ...
        double disX = 0.12;        // x-offset of circle center (negative/positive shapes camber)
        double disY = 0.04;        // y-offset of circle center
        double backFat = 1.02;     // 'backFat' scaling parameter used in generation
        double AoA_deg = 20.0;    // angle of attack in degrees (used as alpha root default)
