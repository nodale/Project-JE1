        ...
        std::vector<double>& alpha_out)
{
    ...
    // -------------------------
    //     Radial stations (cosine spacing)
    // -------------------------
    r_out.resize(N);
    for (int m = 0; m < N; ++m) {
        double theta = (m + 1) * M_PI / (N + 1); // 0..pi
        r_out[m] = r0 + s * 0.5 * (1.0 - std::cos(theta));
    }
    ...
    alpha_out.resize(N);
    for (int m = 0; m < N; ++m) {
        double frac = (r_out[m] - r0) / s;
        alpha_out[m] = alpha_root_deg + (alpha_tip_deg - alpha_root_deg) * frac;
    }

    // -------------------------
    //     Compute Γ(r) using elliptical distribution
    // -------------------------
    std::vector<double> Gamma(N);

    // Lambda for hub-heavy / steep-drop lift fraction with minimum fraction
    double Gamma_min_frac = 0.002; // fraction of max at tip
    auto hub_steep_L_frac = [&](double r) {
        double frac = (r - r0) / s;   // 0 at hub, 1 at tip
        double p = 2.0;               // >1 gives steep initial drop
        double normalized = std::pow(1.0 - frac, p);
        // Shift and scale so tip is at Gamma_min_frac
        return Gamma_min_frac + (1.0 - Gamma_min_frac) * normalized;
    };

    // Compute unnormalized L fractions
    std::vector<double> L_frac(N);
    double sum_L_frac = 0.0;
    for (int m = 0; m < N; ++m) {
        L_frac[m] = hub_steep_L_frac(r_out[m]);
        std::cout << "L_frac[" << m << "] = " << L_frac[m] << std::endl;
        sum_L_frac += L_frac[m];
    }

    // Normalize so total lift per blade equals Lprime_const_Nperm * s
    for (int m = 0; m < N; ++m) {
        double L_section = (L_frac[m] / sum_L_frac) * (Lprime_const_Nperm * s);
        Gamma[m] = L_section / (rho * Vinf);
        std::cout << "Gamma[" << m << "] = " << Gamma[m] << std::endl;
    }

    std::cout << "" << std::endl;

    // -------------------------
    //     Compute chord directly from Γ(r)
    // -------------------------
    chord_out.resize(N);
    const double denom_eps = 1e-6;

    for (int m = 0; m < N; ++m) {
        double alpha_rad = alpha_out[m] * DegreeToRad;
        double denom = 2.0 * (alpha_rad - alpha_L0_rad);
        if (std::abs(denom) < denom_eps) denom = (denom >= 0.0) ? denom_eps : -denom_eps;

        double c_local = (8.0 * M_PI * r_out[m] * Gamma[m]) / (Vinf * denom);

        std::cout << c_local << std::endl;
        // Clip to physical limits
        if (!std::isfinite(c_local) || c_local <= 0) c_local = c_min;
    ...
    std::cout << "Wrote constant_gamma_chord.dat (" << N
        << " stations) using elliptical Gamma(r) distribution\n";
...
int main()
{
    try {
        ...
        double disX = 0.10;        // x-offset of circle center (negative/positive shapes camber)
        double disY = 0.06;        // y-offset of circle center
        ...
        double r0 = 0.01;          // hub radius (m)
        double r1 = 0.12;           // tip radius (m)
        int nRadial = 40;           // number of radial collocation stations for lifting-line
        double rpm = 6400.0;        // rotational speed (kept for potential later use)

        double rho = 1.225;         // air density
        double T = 15.0;            // rotor thrust in N (example)
