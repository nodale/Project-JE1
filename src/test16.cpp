...
std::vector<std::vector<Vec3>> buildBladeGeometry(
        ...
        double offset_root_frac,    // NEW
        double offset_tip_frac )    // NEW
{
    int NR = r.size();
    ...
    for(int i=0; i<NR; ++i)
    {
        ...
        // --- compute linear center offset ---
        double frac   = (r[i] - r.front()) / (r.back() - r.front());
        double cof    = offset_root_frac + (offset_tip_frac - offset_root_frac) * frac;
        double xshift = cof * c;  // physical shift (m)

        for(int j=0; j<NP; ++j)
        {
            // local airfoil coordinates, but shifted by xshift
            double x = baseAirfoil[j][0] * c - xshift;   // SHIFT APPLIED HERE
        ...
        const std::vector<std::vector<Vec3>>& bladeRings)
{
    ...
    // --------------------------------------------------
    // 1) Surface: connect all rings (UNCHANGED)
    // --------------------------------------------------
    ...
    closeRingUpDown(out, bladeRings[0]);       // root
    closeRingUpDown(out, bladeRings[NR-1]);    // tip
                                               //
    out << "endsolid blade\n";
    out.close();
}
