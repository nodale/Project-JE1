        ...
        double rpm, std::vector<double> px, std::vector<double> py)
{
    ...
    for (int m = 0; m < N; ++m)
    {
        ...
        double c_local = Lprime_r / (dyn * Cl_local);
...
int main()
{
    try {
        ...
        double r0 = 0.004;          // hub radius (m)
        double r1 = 0.14;           // tip radius (m)
        int nRadial = 40;           // number of radial collocation stations for lifting-line
        double rpm = 2500.0;        // rotational speed (kept for potential later use)

        double rho = 1.225;         // air density
        double T = 240.0;            // rotor thrust in N (example)
        ...
        double alpha_root_deg = 6.0;
        double alpha_tip_deg = 2.0; 
