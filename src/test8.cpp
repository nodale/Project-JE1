...
// Lambda for normalized elliptical lift fraction (highest at hub)
auto elliptical_L_frac = [&](double r) {
    double frac = (r - r0) / s;       // 0 at hub, 1 at tip
    return std::sqrt(1.0 - frac * frac);
};
...
for (int m = 0; m < N; ++m) {
    L_frac[m] = elliptical_L_frac(r_out[m]);
    sum_L_frac += L_frac[m];
}
...
int main()
{
    try {
        ...
        double disY = 0.04;        // y-offset of circle center
        double backFat = 1.02;     // 'backFat' scaling parameter used in generation
        double AoA_deg = 15.0;    // angle of attack in degrees (used as alpha root default)

        // blade radial / lifting-line parameters
        double r0 = 0.04;          // hub radius (m)
        double r1 = 0.12;           // tip radius (m)
        int nRadial = 20;           // number of radial collocation stations for lifting-line
